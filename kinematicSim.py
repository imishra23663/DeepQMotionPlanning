...
if __name__ == "__main__":
    ...
    startTime = time.time()
    oldTime = startTime
    rotations = np.arange(-np.pi, np.pi, 2*np.pi/8)
    print(rotations)
    time.sleep(3)
    step_size = 0.01
    world_limit = [1, 1]
    ...
    while vis.shown():
        ...
        random_X_rotation = rotations[np.int(np.random.uniform(0, rotations.shape[0]))]
        q = robot.getConfig()
        if q[0] > world_limit[0]:
            q[0] = 0
            q[1] = 0

        if q[1] > world_limit[0]:
            q[0] = 0
            q[1] = 0

        q[0] = q[0] + step_size * np.cos(random_X_rotation)
        q[1] = q[1] + step_size * np.sin(random_X_rotation)
        q[2] = 0.1
        q[3] = 0
        q[4] = 0
        q[5] = 0
        ...
        if np.abs(q[0]-goal[0]) < 0.25 and np.abs(q[1]-goal[1]) < 0.25:
            vis.addText("textCol", " Goal Reached")
            vis.unlock()
            vis.setColor("textCol", 0.1500, 0.3250, 0.0980)
            time.sleep(3)
            continue
